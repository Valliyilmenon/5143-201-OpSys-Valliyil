...
class Process(object):
    ...
    - **Attributes**:
        ...
        - process_id      (int)   : Unique identification for each of the process in the operating
                                    system.
        - state           (enum)  : Indicates whether the process in New, or is deployed by the CPU
                                    from ready queue level1 or level2
    ...
    def __init__(self, **kwargs):
        ...
        self.process_id = 0
        self.time = 0
        self.start_time = None  # Indicate job start time (by the CPU)
        self.com_time = None  # Indicate job completion time
        self.rem_from_sq = 0  # Indicate the time when the job was removed from the job_scheduling_queue
        ...
        if 'complete_cpu_cycles' in kwargs:
            self.complete_cpu_cycles = kwargs['complete_cpu_cycles']
        if 'time' in kwargs:
            self.time = kwargs['time']
        # if 'rem_from_sq' in kwargs:
        #     self.rem_from_sq = kwargs['rem_from_sq']
