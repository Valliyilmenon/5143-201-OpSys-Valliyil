...
class SemaphoreQueues(object):

    def __init__(self):

        num_of_sems = 4
        sem_queue = []
        for i in range(num_of_sems):  # Create a list of Fifo objects, one for each semaphore (waiting queue)
            sem_queue.append(Fifo())

        self.sem_ready_queue = Fifo()
